...
ZXVLI_OPDIV
{
  size_t i = 0, pi = 0, I = 0, end = val->m_bits, size = zxvli.size( src );
  ...
  zxuc bitl = 1u << (CHAR_BIT - 1), bit = bitl,
    ...
    *REM, *DST, *QUO, *CPY;
  if ( remainder )
    mset( remainder->m_data, 0, zxvli.size( remainder ) );
  if ( !src )
    return NULL;
  ...
  if ( remainder )
  {
    ...
    zxv.grow( &dst.m_core, size & ZXUINT_MAX, NULL );
    *remainder = dst;
  }
  else
    dst.m_data = (zxuc*)malloc( size );
  quo.m_data = (zxuc*)malloc( size );
  cpy.m_data = (zxuc*)malloc( size );
  rem.m_data = (zxuc*)malloc( size );
  DST = (zxuc*)dst.m_data;
  QUO = (zxuc*)quo.m_data;
  CPY = (zxuc*)cpy.m_data;
  REM = (zxuc*)rem.m_data;
  ...
  for ( ; i != pi && zxvli.cmpME( src, val ); --i, --pi )
  {
    ...
    if ( zxvli.cmpME( &dst, val ) )
    {
      zxvli.opSub( &dst, val );
      QUO[ 0 ] |= 1u;
      mcpy( cpy.m_data, dst.m_data, size );
      zxvli.opMvl( &cpy, i );
      zxvli.opXor( src, &rem );
      zxvli.opOr(  src, &cpy );
    }
